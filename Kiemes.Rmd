...
for (z in 1:length(levels(GESIS_panel$zufriedenheit))) {
  plots_[[z]] <- ggplot(
    GESIS_panel %>% 
      filter(zufriedenheit == levels(GESIS_panel$zufriedenheit)[[z]]) %>% 
      select(fahrrad), aes(x = fahrrad)
    ) +
  geom_bar(aes(y = (..count..)/sum(..count..))) +
  scale_x_discrete(labels = c(1:5),limits = rev) +
  theme_bw() +
  labs(x = "Fahrradnutzung",
       y = "Prozent")
  
  plots_[[z]] <- plots_[[z]] + ggtitle(levels(GESIS_panel$zufriedenheit)[[z]])
}

plots_[[1]] + plots_[[2]] + plots_[[3]]
```
...
ggplot(GESIS_panel, aes(x = fahrrad)) +
  ...
  labs(
    ...
    caption = "1: (Fast) nie 
    2: Seltener 
    3: An 1 bis 3 Tagen pro Monat 
    4: An 1 bis 3 Tagen pro Woche 
    5: (Fast) taeglich"
  )
