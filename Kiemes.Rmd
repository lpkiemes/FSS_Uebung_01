...
```
Die Variable `bcan111a` misst die gegenwaertige Zufriedenheit von 0 bis 10 Punkten. Die dazugehoerige single choice Frage lautet: "Wie zufrieden sind Sie - alles in allem - mit Ihrem gegenwärtigen Leben?".
  ...
  mutate(
    fahrrad = factor(
      ...
      labels = c("(Fast) taeglich", "An 1-3 Tagen/Woche", 
                 "An 1-3 Tagen/Monat", "Seltener", "(Fast) nie") # 5 Kategorien
      )
    )
```
...
ggplot(GESIS_panel, aes(x = fahrrad)) +
  ...
  labs(
    title = "Gruppierte Lebenszufriedenheit und Fahrradnutzung",
    x = "Fahrradnutzung",
    y = "Anzahl",
    subtitle = "n = 595"
  ) +
...
ggplot(GESIS_panel, aes(x = fahrrad, group = zufriedenheit)) +
  ...
  facet_wrap(~ zufriedenheit) +
  labs(
    title = "Fahrradnutzung nach Zufriedenheit",
    x = "Fahrradnutzung",
    y = "Prozent"
  ) +
  coord_flip()
```
```{r, fig.dim = c(8, 3)}
ggplot(GESIS_panel, aes(x = zufriedenheit, group = fahrrad)) +
  geom_bar(aes(y = ..prop..), stat = "count", fill = "lightblue") +
  scale_y_continuous(labels = scales::percent) +
  scale_x_discrete(labels = levels(GESIS_panel$fahrrad)) +
  facet_wrap(~ fahrrad) +
...
```{r bar plot nach zufriedenheit}
ggplot(GESIS_panel, aes(x = fahrrad, group = zufriedenheit)) +
  geom_bar(aes(y = ..prop.., fill = factor(..x..)), stat = "count") +
  scale_y_continuous(labels = scales::percent) +
  scale_x_discrete(limits = rev) +
  scale_fill_discrete(name = c("Fahrradnutzung"), labels = rev(levels(GESIS_panel$fahrrad))) +
  theme(axis.text.x=element_blank(), # ohne achsen label
        axis.ticks.x=element_blank()) + # ohne achsen ticks
  facet_grid(~ zufriedenheit) +
  labs(
    title = "Fahrradnutzung nach Zufriedenheit",
    x = "Fahrradnutzung",
    y = "Prozent"
...
```{r bar plot nach fahrrad}
my_palette <- brewer.pal(name = "Oranges", n = 9)[5:9]

ggplot(GESIS_panel, aes(x = fahrrad, fill = fahrrad)) +
...
```{r jitter nach zufriedenheit}
ggplot(GESIS_panel, aes(x = zufriedenheit, y = fahrrad)) +
  geom_jitter(alpha = 0.5, color = "navyblue") +
  scale_y_discrete(limits = rev) +
  labs(
    title = "Zusammenhang zwischen Fahrradnutzung  und Lebenszufriedenheit",
    x = "Zufriedenheit",
    y = "Fahrradnutzung"
  )
```
